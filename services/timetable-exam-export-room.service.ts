...
export async function exportRoomScheduleExam(
  req: FastifyRequest,
  res: FastifyReply
) {
  ...
  let room = data.reduce<Record<string, (typeof data)[number]["exam"]["room"]>>(
    (acc, curr) => {
      if (curr.exam.room && !acc[curr.exam.room.id]) {
        acc[curr.exam.room.id] = curr.exam.room;
      }
      return acc;
    },
    {}
  ...
  Object.values(room).forEach((vRoom) => {
    const { id: roomId, name: roomName, building } = vRoom!;

    const { ws, styleCenter, styleBorder } = createWorksheetLayout(
      `${building.code}-${roomName}`
    );
    ...
    data.forEach((vProcessed) => {
      if (vProcessed.exam.room?.id !== roomId) return;

      const associatedSubject = vProcessed.exam.section[0].subject;
      const associatedExamSectionNo = vProcessed.exam.section.map(
        (obj) => obj.no
      );
      ...
      ws.cell(
        ...
        .string(
          associatedSubject.code +
            "_SEC_" +
            associatedExamSectionNo.join(", ") +
