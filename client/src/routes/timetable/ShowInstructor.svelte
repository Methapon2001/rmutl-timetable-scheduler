<script lang="ts">
  ...
  function detectLocalOverlap(current: typeof state, localInstructor: (typeof instructor)[number]) {
    ...
    if (localState.section) {
      localState.section.instructor = [localInstructor];
    }

    localState.isOverflow = localState.period + localState.size - 1 > 25;

    const { isOverlap, overlapGroup, overlapInstructor, overlapRoom, overlapSubject } =
  ...
  let selectedInstructor = state.section?.instructor.map((v) => v.id) ?? [];
